...
for x in input:
    ...
    for y in x:
        ...
        if diffrence_col == 2 and column_set == False:
            if y == "F":
                column = lower_column
            elif y == "B":
                column = upper_column
            column_set = True
        elif difference_seat == 2 and seat_set == False:
            if y == "L":
                seat = lower_seat
            ...
            upper_column -= math.floor(diffrence_col / 2.0)

        elif y == "B":
            lower_column += math.ceil(diffrence_col / 2.0)

        elif y == "L":
            upper_seat -= math.floor(difference_seat / 2.0)
